...
export default function AdminSettings() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('crowdex_settings');
    if (saved) {
      try {
  ...
  const handleSave = () => {
    setLoading(true);
    setTimeout(() => {
      localStorage.setItem('crowdex_settings', JSON.stringify(settings));
      setLoading(false);
  ...
  const handleReset = () => {
    if (confirm('Reset all settings to default?')) {
      ...
      localStorage.removeItem('crowdex_settings');
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      >
        ...
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => { localStorage.removeItem('crowdex_cache'); toast({ title: 'Cache cleared' }); }}>
